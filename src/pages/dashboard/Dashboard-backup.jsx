...
// Modify the api axios instance to include better error handling
...
const Dashboard = () => {
  ...
  // Modify the fetchOrders function to use the safe wrapper
  const fetchOrders = async () => {
    return safeApiCall(async () => {
      ...
      const mappedOrders = ordersList.map(order => ({
        ...order,
        total: order.calculated_total || order.total || order.total_amount || 
  ...
  useEffect(() => {
    ...
    const silenceTrackingErrors = () => {
      // Create a modified fetch that doesn't log certain 404 errors
        ...
        try {
          ...
          orders.forEach(order => {
            ...
            if (status.includes('baru')) {
              baru++;
            } else if (status.includes('proses') || status.includes('produksi')) {
              proses++;
            } else if (status.includes('selesai') || status.includes('terkirim')) {
              selesai++;
            }
            
            // Hitung pendapatan
            if (order.total_amount) {
              pendapatan += parseFloat(order.total_amount) || 0;
            } else if (order.items && Array.isArray(order.items)) {
              // Hitung dari items jika total_amount tidak ada
              order.items.forEach(item => {
                const itemTotal = (parseFloat(item.unit_price) || 0) * (parseInt(item.quantity) || 1);
                pendapatan += itemTotal;
              });
            }
          });
